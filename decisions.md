...
- **Main is merge-only.** No direct commits to main through the UI or API — every change flows branch → compare → merge, so nothing ever lands without passing the conflict rules and the validator, and the merge history *is* the change history. (The seed template is a server-side exception so a fresh install isn't stuck.) Rejected alternative: git's "commit to main if you like" looseness — cheaper to demo, but it lets exactly the unreviewed drift this product exists to prevent.
